...
//#ifndef __CC_ARM
extern const uint8_t __user_flash_start;
extern const uint8_t __user_flash_size;
//#else
// static const uint32_t *_real_flash_size = (uint32_t *)0x7c000;
//const uint8_t __user_flash_start __attribute__((at(0x4000)));
//const uint8_t __user_flash_size __attribute__((at(0x7c000)));
//#endif
...
void DFU_Download(CONTROL_TRANSFER *control)
{
	...
	flash_p = (&__user_flash_start) + (control->setup.wValue * DFU_BLOCK_SIZE);

	if (control->setup.wLength > 0)
	{
		if ((flash_p + control->setup.wLength) <= ((&__user_flash_start) + ((uint32_t)(&__user_flash_size))))
		{
