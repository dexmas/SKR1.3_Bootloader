...
int SDCard_initialise_card() {
    ...
    if(SDCard__cmd(SDCMD_GO_IDLE_STATE, 0) != R1_IDLE_STATE) {
        printf(stderr, "Could not put SD card in to SPI idle state\n");
    ...
    if(r == R1_IDLE_STATE) {
        ...
        printf(stderr, "Not in idle state after sending CMD8 (not an SD card?)\n");
        return cardtype = SDCARD_FAIL;
    }
}
...
int SDCard_initialise_card_v1() {
    ...
    printf(stderr, "Timeout waiting for v1.x card\n");
    return SDCARD_FAIL;
}
...
int SDCard_initialise_card_v2() {
    ...
    printf(stderr, "Timeout waiting for v2.x card\n");
    return cardtype = SDCARD_FAIL;
}
...
int SDCard_disk_initialize()
{
	...
	if(SDCard__cmd(SDCMD_SET_BLOCKLEN, 512) != 0) {
        printf(stderr, "Set 512-byte block timed out\n");
        return 1;
    }
...
uint32_t SDCard__sd_sectors()
{
    ...
    if(SDCard__cmdx(SDCMD_SEND_CSD, 0) != 0) {
        printf(stderr, "Didn't get a response from the disk\n");
        return 0;
    }

    uint8_t csd[16];
    if(SDCard__read(csd, 16) != 0) {
        printf(stderr, "Couldn't read csd response from disk\n");
        return 0;
    }
